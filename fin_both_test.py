...
def main(input_file, track_mode, filter_option, association_type):
    ...
    association_method = association_type  # 'JPDA', 'Munkres', or 'Both'
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        if len(group) == 1:  # Single measurement
            ...
            elif association_method == 'Both':
                # Perform both JPDA and Munkres
                clusters_jpda, best_reports_jpda, hypotheses_jpda, probabilities_jpda = perform_jpda(
                    [track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter
                )
                best_reports_munkres = perform_munkres([track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter)

                # Log JPDA and Munkres results
                log_data_jpda = {
                    'Time': group[0][3],
                    'Association Type': 'JPDA',
                    'Clusters Formed': clusters_jpda,
                    'Hypotheses Generated': hypotheses_jpda,
                    'Probability of Hypothesis': probabilities_jpda,
                    'Best Report Selected': best_reports_jpda
                }
                log_to_csv(log_file_path, log_data_jpda)

                log_data_munkres = {
                    'Time': group[0][3],
                    'Association Type': 'Munkres',
                    'Best Report Selected': best_reports_munkres
                }
                log_to_csv(log_file_path, log_data_munkres)

                # Compare and select best reports
                best_reports = best_reports_jpda + best_reports_munkres
...
class KalmanFilterGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.both_radio = QRadioButton("Both")  # New option for both JPDA and Munkres
        association_layout.addWidget(self.both_radio)
    ...
    def process_data(self):
        ...
        association_type = "JPDA" if self.jpda_radio.isChecked() else "Munkres" if self.munkres_radio.isChecked() else "Both"
