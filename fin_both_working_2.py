...
def perform_combined_association(tracks, reports, kalman_filter):
    # Perform JPDA
    jpda_clusters, jpda_best_reports, jpda_hypotheses, jpda_probabilities = perform_jpda(tracks, reports, kalman_filter)

    # Perform Munkres
    munkres_best_reports = perform_munkres(tracks, reports, kalman_filter)

    # Log detailed report for both JPDA and Munkres
    detailed_report = []
    for track_id, (jpda_report, munkres_report) in enumerate(zip(jpda_best_reports, munkres_best_reports)):
        detailed_report.append({
            'Track ID': track_id,
            'JPDA Report': jpda_report,
            'Munkres Report': munkres_report
        })

    # Save detailed report to CSV
    with open('combined_association_report.csv', 'w', newline='') as csvfile:
        fieldnames = ['Track ID', 'JPDA Report', 'Munkres Report']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for row in detailed_report:
            writer.writerow(row)

    return jpda_best_reports, munkres_best_reports
...
def main(input_file, track_mode, filter_option, association_type):
    ...
    association_method = association_type  # 'JPDA', 'Munkres', or 'Combined'
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        if len(group) == 1:  # Single measurement
            ...
            # Initialize best_reports with an empty list
            best_reports = []
            ...
            elif association_method == 'Combined':
                jpda_best_reports, munkres_best_reports = perform_combined_association(
                    [track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter
                )
                # Decide how to combine or choose between jpda_best_reports and munkres_best_reports
                # For example, you might want to use one of them as the final best_reports
                best_reports = jpda_best_reports  # or munkres_best_reports, depending on your logic
...
class KalmanFilterGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.combined_radio = QRadioButton("Combined")  # New option for combined association
        association_layout.addWidget(self.combined_radio)
    ...
    def process_data(self):
        ...
        association_type = "JPDA" if self.jpda_radio.isChecked() else "Munkres" if self.munkres_radio.isChecked() else "Combined"
